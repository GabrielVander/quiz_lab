...
/// ```
/// {@endtemplate}
extension AsyncResult<T, E> on Result<T, E> {
  //------------------------------------------------------------------------
  // Methods
  ...
  ///
  /// ```dart
  /// Future<Result<int, String>> parseToInt(String value) {
  ///   try {
  ///     return Ok(int.parse(value));
  ///   } catch (_) {
  ///     return Err(value);
  ...
  ///   .map((val) => val * val);
  /// ```
  FutureOr<Result<T, E>> inspectAsync(FutureOr<void> Function(T value) f) {
    if (runtimeType == Ok<T, E>) return (this as Ok<T, E>).inspectAsync(f);

    return (this as Err<T, E>).inspectAsync(f);
  }
...
// class OkAsync<T, E> implements AsyncResult<T, E> {
//   OkAsync({required this.value, required this.result});
//
//   final T value;
//   final Ok<T, E> result;
//
...
//   @override
//   FutureOr<Result<T, E>> inspectAsync(FutureOr<void> Function(T value) f) async {
//     await f(value);
//
//     return result;
//   }
...
extension OkAsyncExt<T, E> on Ok<T, E> {
  FutureOr<Result<T, E>> inspectAsync(FutureOr<void> Function(T value) f) async {
    await f(v);
